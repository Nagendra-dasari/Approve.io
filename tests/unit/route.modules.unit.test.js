...
jest.mock("../../src/models/externalUser.model", () => ({ create: jest.fn() }));
jest.mock("../../src/models/publicFormToken.model", () => ({ create: jest.fn(), findOne: jest.fn() }));
jest.mock("../../src/models/kycRecord.model", () => ({ create: jest.fn(), findOne: jest.fn() }));
...
describe("route modules unit coverage", () => {
  ...
  it("covers kyc/signature/document/notification/import/audit handlers", async () => {
    KycRecord.create.mockResolvedValue({ _id: "k1" });
    KycRecord.findOne.mockResolvedValue({ _id: "k1", status: "PENDING", save: jest.fn() });
    ...
    Notification.find.mockReturnValue({ sort: jest.fn().mockResolvedValue([{ _id: "n1" }]) });
    ...
    await callRouteHandler(kycRoutes, "/initiate", "POST", { body: { refType: "SUBMISSION", refId: "s1", aadhaarNumber: "1" } });
    await callRouteHandler(kycRoutes, "/verify", "POST", { body: { kycId: "k1", providerRef: "p1", otp: "123456" } });
    ...
    await callRouteHandler(notificationsRoutes, "/", "POST", { body: { channel: "EMAIL", message: "x", eventType: "E", to: "a@x.com" } });
    ...
    const badKyc = await callRouteHandler(kycRoutes, "/verify", "POST", { body: { kycId: "k1" } });
    expect(badKyc.next).toHaveBeenCalled();
